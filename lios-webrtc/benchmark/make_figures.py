...
def fig_throughput():
    ...
    ax.annotate("LiOS: no saturation up to 1200 fps\n(receiver GIL limit, not pipeline limit)",
                xy=(1200, 1202), xytext=(520, 1095), fontsize=10.5, color="#0f5e52", fontweight="bold")
    ax.annotate(f"≥{LK_BREAK} fps: subscriber stream interrupted",
                xy=(LK_BREAK, 0), xytext=(610, 95), fontsize=10.5, color=LK, fontweight="bold")
    ax.set_xlabel("Target frame rate (fps)", fontsize=12)
    ax.set_ylabel("Achieved frame rate (fps)", fontsize=12)
    ax.set_xlim(0, 1260); ax.set_ylim(0, 1300)
    ax.grid(ls=(0, (3, 4)), alpha=0.35)
    ax.set_title("Single-stream decode throughput: achieved vs. target\n"
    ...
    fig.text(0.5, 0.013,
             ...
             ha="center", va="bottom", fontsize=8.0, color="#777")
...
def fig_jitter():
    ...
    for i, pc in enumerate(parts["bodies"]):
        pc.set_facecolor([GST, LK][i]); pc.set_alpha(0.55); pc.set_edgecolor([GST, LK][i])
    for k in ("cmedians", "cmaxes", "cmins", "cbars"):
        if k in parts:
            parts[k].set_edgecolor("#333"); parts[k].set_linewidth(1.2)
    ...
    ax.set_xticks([1, 2]); ax.set_xticklabels(["LiOS", "LiveKit"], fontsize=12.5)
    ax.set_ylabel("Inter-frame arrival interval (ms)", fontsize=12)
    ax.set_ylim(0, ymax)
    ax.grid(axis="y", ls=(0, (3, 4)), alpha=0.35)
    ax.set_title("Inter-frame arrival interval · identical clip @400 fps · localhost\n"
...
def fig_latency():
    ...
    for fn, lab, c, e in SERIES:
        ...
        if not vals:
            continue
        m = st.median(vals)
        labels.append(lab); meds.append(m); los.append(m - min(vals)); his.append(max(vals) - m)
        colors.append(c); edges.append(e); n_runs = len(vals)
    ...
    for xi, (m, h) in enumerate(zip(meds, his)):
        ax.text(xi, m + h + ymax * 0.02, f"≈{round(m)} ms", ha="center", va="bottom",
    ...
    ax.text(0.5, 1.005, f"bidirectional NTP method (clock offset cancelled) · {n_runs} runs/group · error bars = measured min–max",
...
def fig_arch():
    ...
    def zone(x, y, w, h, bg, bd, title, tc):
        ax.add_patch(FancyBboxPatch((x, y), w, h, boxstyle="round,pad=0.04,rounding_size=0.18", fc=bg, ec=bd, lw=2.2, zorder=1))
    ...
    def node(cx, cy, w, h, text, bg="#fff", bd="#888", fs=11, bold=False, tc="#222"):
        ax.add_patch(FancyBboxPatch((cx - w / 2, cy - h / 2), w, h, boxstyle="round,pad=0.02,rounding_size=0.10", fc=bg, ec=bd, lw=1.5, zorder=4))
        ax.text(cx, cy, text, ha="center", va="center", fontproperties=(fpb if bold else fp), fontsize=fs, color=tc, zorder=5)
        return (cx, cy, w, h)

    def varrow(a, b, color="#555", lw=2.0):
        ax.add_patch(FancyArrowPatch((a[0], a[1] - a[3] / 2), (b[0], b[1] + b[3] / 2), arrowstyle="-|>", mutation_scale=15, lw=lw, color=color, zorder=3))

    ax.text(7.75, 7.62, "LiOS image-transmission architecture", ha="center", va="center", fontproperties=fpb, fontsize=21, color="#1a1a1a")
    ax.text(7.75, 7.2, "End-to-end GPU-resident  ·  NVENC/NVDEC hardware codec  ·  WebRTC real-time transport", ha="center", va="center", fontproperties=fp, fontsize=12, color="#666")
    ax.add_patch(FancyBboxPatch((0.4, 6.25), 14.7, 0.62, boxstyle="round,pad=0.02,rounding_size=0.12", fc="#f0ecf9", ec=SIG_BD, lw=1.8, zorder=1))
    ax.text(7.75, 6.56, "WebSocket signaling  ·  SDP / ICE  ·  multi-camera negotiation", ha="center", va="center", fontproperties=fpb, fontsize=12, color=SIG_BD)
    ZY, ZH = 0.55, 5.05
    EX, EW = 0.4, 4.4
    CX, CW = 10.7, 4.4
    ...
    c3 = node(cx, 1.95, nw, 0.82, "CUDA memory pool\nCUDA tensor -> VLA inference", bg="#d9efe2", bd=CLOUD_BD, fs=10.5, bold=True)
    varrow(c1, c2); varrow(c2, c3)
    RX, RW = 5.35, 4.8
    ax.add_patch(FancyBboxPatch((RX, 2.85), RW, 1.55, boxstyle="round,pad=0.04,rounding_size=0.22", fc=RELAY_BG, ec=RELAY_BD, lw=2.2, zorder=1))
    ax.text(RX + RW / 2, 4.08, "Public relay", ha="center", va="center", fontproperties=fpb, fontsize=13, color=RELAY_BD)
    node(RX + RW / 2, 3.35, 4.0, 0.62, "coturn  TURN / STUN\nSRTP over UDP", bg="#fbe9cf", bd=RELAY_BD, fs=10.5, bold=True)
    ax.add_patch(FancyArrowPatch((e3[0] + e3[2] / 2, e3[1] + 0.1), (RX + 0.1, 3.55), arrowstyle="-|>", mutation_scale=18, lw=2.6, color="#c0392b", connectionstyle="arc3,rad=-0.15", zorder=6))
    ax.add_patch(FancyArrowPatch((RX + RW - 0.1, 3.55), (c1[0] - c1[2] / 2, c1[1] - 0.1), arrowstyle="-|>", mutation_scale=18, lw=2.6, color="#c0392b", connectionstyle="arc3,rad=-0.15", zorder=6))
    ax.text(RX + RW / 2, 4.62, "encrypted real-time media (SRTP)", ha="center", va="center", fontproperties=fp, fontsize=10, color="#c0392b")
    for sx, stop in [(ex, e3[1] + e3[3] / 2), (cx, c1[1] + c1[3] / 2)]:
        ax.add_patch(FancyArrowPatch((sx, 6.25), (sx, stop), arrowstyle="-|>", mutation_scale=11, lw=1.3, color=SIG_BD, ls=(0, (4, 3)), zorder=2))
    ax.text(7.75, 0.18, "Red nodes stay resident in GPU memory  -  no CPU<->GPU round-trip", ha="center", va="center", fontproperties=fpb, fontsize=11.5, color=GPU_BD)
...
if __name__ == "__main__":
    ...
    ap.add_argument("figure", nargs="?", default="all", choices=["all"] + list(FIGS),
